...
compile_move() {
   ...
   cd $1
   aptos move compile --move-2 --bytecode-version 6 --save-metadata --named-addresses apto_orm=$APTO_ORM_ADDR
   cd - >> /dev/null
}
...
test_move() {
   _check_move_package $1
   cd $1
   aptos move test --move-2 --bytecode-version 6 --named-addresses apto_orm=0x1e51 --ignore-compile-warnings || exit 1
   cd - >> /dev/null
}
...
publish_move() {
   ...
   # aptos move publish --move-2 --override-size-check --bytecode-version 6 --profile default --named-addresses apto_orm=$APTO_ORM_ADDR --assume-yes
   aptos move publish --move-2 --bytecode-version 6 --profile default --named-addresses apto_orm=$APTO_ORM_ADDR --assume-yes || exit 1
   cd - >> /dev/null
}
...
function orm_publish_local() {
   ...
   cd "move/utilities"
   aptos move publish --move-2 --assume-yes --bytecode-version 6 --private-key-file ../../.key/default \
