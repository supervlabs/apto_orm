...
import orm, { OrmStruct, OrmClass, OrmField, snakeToCamel } from '../sdk';
import path from 'path';
import fs from 'fs';

const package_name = 'list_field';
const package_creator = orm.loadAccountFromPrivatekeyFile('../.key/user');
const package_move_path = path.join(__dirname, '.move/list_field');

@OrmStruct({
  package_name,
  package_creator,
  module_name: 'gacha_rounds',
})
class CachaDropRate {
  // [common, advanced, rare, hero, legend]

  @OrmField({ type: 'u32' })
  numerator_for_advanced!: number;

  @OrmField({ type: 'u32' })
  numerator_for_rare!: number;

  @OrmField({ type: 'u32' })
  numerator_for_hero!: number;

  @OrmField({ type: 'u32' })
  numerator_for_legend!: number;

  denominator_for_all: number;
}
...
class GachaRounds {
  ...
  @OrmField()
  rounds!: CachaDropRate[];
}

describe('test the struct type fields', () => {
  beforeEach(() => {});
  ...
  it('Test to define, generate, compile, publish and create AptoORM Object', async () => {
    ...
    const package_config: orm.OrmPackageConfig = {
      ...
      package_name,
      package_move_path,
      ormobjs: [GachaRounds, CachaDropRate],
      local_apto_orm_package: path.join(__dirname, '../../../move/apto_orm'),
    };
    orm.generatePackage(package_config);
    expect(fs.existsSync(`${package_move_path}/sources/gacha_rounds.move`)).toBe(true);
    ...
    // const classAddr = orm.getClassAddress(GachaRounds).toString();
    // expect(classAddr).toBeDefined();
    // expect(orm.getClassAddress(a).toString()).toBe(classAddr);
    // expect(orm.getClassAddress('GachaRounds').toString()).toBe(classAddr);

    // const c: GachaRounds = new GachaRounds({
    //   title: '3th title',
    ...
    // expect(async () => {
    //   await client.getObject({ object: GachaRounds, address: a_address }, true);
    // }).not.toThrow();
    ...
    // const b_address = client.retrieveOrmObjectAddressFromTxnr(txnr, { object_type: GachaRounds });
    // console.log('createTxn', txnr.hash);
    // console.log('retrieveOrmObjectAddressFromTxnr', b_address);
    // expect(async () => {
    //   await client.getObject({ object: GachaRounds, address: b_address }, true);
    // }).not.toThrow();
    ...
    // expect(async () => {
    //   await client.getObject({ object: GachaRounds, address: c_address }, true);
    // }).not.toThrow();
